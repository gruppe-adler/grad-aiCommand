...
if (hasInterface) then {
    _action = ["grad_aicommand_actionNode", "AI command", "\A3\ui_f\data\igui\cfg\simpleTasks\types\whiteboard_ca.paa", {_this call FUNC(waitASecond)}, {!((group (_this select 0)) getVariable [QGVAR(isBeingEdited),false]) && _this call FUNC(canGiveCommands)}, {},[],"",0,[false, false, false, true, false]] call ace_interact_menu_fnc_createAction;
    ...
    _action = ["grad_aicommand_highcommandAction", "AI command", "\A3\ui_f\data\igui\cfg\simpleTasks\types\whiteboard_ca.paa", {_this call FUNC(openCommandWindow)}, {_this call FUNC(canUseHighcommand)}] call ace_interact_menu_fnc_createAction;
